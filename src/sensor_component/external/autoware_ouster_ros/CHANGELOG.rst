...
[unreleased]
============
* Add an Autoware compatible point type: setting ``point_type`` to ``xyzircaedt``
  publishes ``autoware::point_types::PointXYZIRCAEDT`` in the byte exact layout
  Autoware validates against, which preserves intensity, channel and the per
  point timestamps that a mismatched layout would silently drop.
* Add the ``intensity_source`` {reflectivity, signal, near_ir} and
  ``intensity_scale`` parameters that decide which sensor channel becomes the
  uint8 intensity of the ``xyzircaedt`` point type.
* Add ``config/autoware_driver_params.yaml``, a preset for Autoware deployments.
