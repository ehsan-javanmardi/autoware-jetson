    ...
    for (auto u = 0; u < ls.h; u += rows_step) {
        for (auto v = 0; v < ls.w; ++v) {   // TODO[UN]: consider cols_step in future
            ...
            CondBinaryOp<!std::is_same_v<PointT, PointS>>::run(
                cloud.points[tgt_idx], staging_point,
                [&opts](auto& tgt_pt, const auto& src_pt) {
                    point::transform(tgt_pt, src_pt, opts);
                });
            // which return a point belongs to is only known to the caller
            point::set_return_type(cloud.points[tgt_idx], return_type);
        }
    }
}
