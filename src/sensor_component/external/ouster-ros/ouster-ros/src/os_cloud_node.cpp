...
class OusterCloud : public OusterProcessingNodeBase {
   ...
   private:
    ...
    void declare_parameters() {
        ...
        declare_parameter("intensity_source", "reflectivity");
        declare_parameter("intensity_scale", 1.0);
    ...
    void create_publishers_subscriptions(const sensor::sensor_info& info) {
        ...
        if (impl::check_token(tokens, "PCL")) {
            ...
            // point types that carry a uint8 intensity, presently only
            // xyzircaedt, need to be told which channel to source it from
            point::TransformOpts transform_opts;
            transform_opts.intensity_source = point::intensity_source_of(
                get_parameter("intensity_source").as_string());
            transform_opts.intensity_scale = static_cast<float>(
                get_parameter("intensity_scale").as_double());
            ...
            // reflectivity is reported by every profile, signal is not
            if (transform_opts.intensity_source ==
                    point::TransformOpts::IntensitySource::SIGNAL &&
                !PointCloudProcessorFactory::profile_has_intensity(
                    info.format.udp_profile_lidar)) {
                RCLCPP_WARN_STREAM(
                    get_logger(),
                    "intensity_source 'signal' is not available on the udp "
                    "profile: "
                    << to_string(info.format.udp_profile_lidar)
                    << ", intensity will be reported as 0");
            }
        }
