...
- [x] **Health monitor.** `web_ui/` ported from `upstream`, inventory re-targeted to Livox / ZED-F9R / Segway. Serving on :8842.
- [x] **Foxglove bridge.** Installed; allow-list in [`foxglove/topics.yaml`](../foxglove/topics.yaml), generated by `build_whitelist.py`. Serving on :8765.
- [ ] **Foxglove layouts**, versioned in [`foxglove/layouts/`](../foxglove/layouts). Nothing to reuse from `racing_kart_v2x` — it has no Foxglove configuration at all.
...
- [x] **Operation buttons** — engage, autonomous/manual, stop and e-stop, in `segway_web_control` on :8843.
